...
public:
    ...
    PlaneSegmenter() : coefficients(new pcl::ModelCoefficients()), planeCloud(new PointCloudT), segmentedCloud(new PointCloudT)
    {
        // Optional
    ...
    void estimatePlane(bool filter = true)
    {
        ...
        if (inliers->indices.size() == 0) {
            ...
            error = true;
            return;   // TODO: use this as error value
        }
        else
            error = false;
...
private:
    ...
    PointCloudTPtr filterCloud(PointCloudTConstPtr cloud) const
    {
        ...
        std::cout << "PointCloud after filtering: " << cloudFiltered->width * cloudFiltered->height << " data points." << std::endl;
